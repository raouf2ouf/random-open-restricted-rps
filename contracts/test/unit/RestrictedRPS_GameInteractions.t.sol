...
contract RestrictedRPS_GameCreationTest is TestUtils {
    ...
    // Game Playing
    //////////////////
    function test_offeringAMatch() public {
        // Create Valid Game
        ...
        // offer match
        (uint16 matchId, bytes32 hashedCard) = offerMatch(
            game,
            player1,
        ...
        RestrictedRPSGame.PlayerState memory playerState = game.getPlayerState(1);

        assert(m.realPlayer1Id == 0);
    ...
    function test_cancellingAMatch() public {
        // Create Valid Game
        RestrictedRPSGame game = createGameWithPlayers(2, 1);
    ...
    function test_OfferingTooManyMatches() public {
        // Create Valid Game
        RestrictedRPSGame game = createGameWithPlayers(2, 1);
    ...
    function test_answeringAMatch() public {
        // Create Valid Game
        RestrictedRPSGame game = createGameWithPlayers(2, 1);
    ...
    function test_closingAMatchLoss() public {
        // Create Valid Game
        RestrictedRPSGame game = createGameWithPlayers(2, 1);

        uint8 p1Bet = 1;
        uint8 p2Bet = 2;
        uint16 matchId = offerAndAnswerAndCloseMatchGivenCards(game, 0, 1, uint8(RestrictedRPSGame.Card.SCISSORS), uint8(RestrictedRPSGame.Card.ROCK), "secret", p1Bet, p2Bet);
        RestrictedRPSGame.Match memory m = game.getMatches()[matchId];
        RestrictedRPSGame.PlayerState memory p1State = game.getPlayerState(1);
        RestrictedRPSGame.PlayerState memory p2State = game.getPlayerState(2);
    ...
    function test_closingAMatchDraw() public {
        // Create Valid Game
        RestrictedRPSGame game = createGameWithPlayers(2, 1);

        uint8 p1Bet = 1;
        uint8 p2Bet = 2;
        uint16 matchId = offerAndAnswerAndCloseMatchGivenCards(game, 0, 1, uint8(RestrictedRPSGame.Card.SCISSORS), uint8(RestrictedRPSGame.Card.SCISSORS), "secret", p1Bet, p2Bet);
        RestrictedRPSGame.Match memory m = game.getMatches()[matchId];
        RestrictedRPSGame.PlayerState memory p1State = game.getPlayerState(1);
        RestrictedRPSGame.PlayerState memory p2State = game.getPlayerState(2);
    ...
    function test_closingAMatchWin() public {
        // Create Valid Game
        RestrictedRPSGame game = createGameWithPlayers(2, 1);

        uint8 p1Bet = 1;
        uint8 p2Bet = 2;
        uint16 matchId = offerAndAnswerAndCloseMatchGivenCards(game, 0, 1, uint8(RestrictedRPSGame.Card.SCISSORS), uint8(RestrictedRPSGame.Card.PAPER), "secret", p1Bet, p2Bet);
        RestrictedRPSGame.Match memory m = game.getMatches()[matchId];
        RestrictedRPSGame.PlayerState memory p1State = game.getPlayerState(1);
        RestrictedRPSGame.PlayerState memory p2State = game.getPlayerState(2);
